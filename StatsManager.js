...
export class StatsManager {
    ...
    async uploadScoreToSteam(finalScore) {
        if (!window.electronAPI || !window.electronAPI.uploadSteamScore) {
            console.log('ℹ Steam API not available (running outside Electron or Steam not ready)');
            return;
        }

        try {
            console.log('📤 Uploading score to Steam leaderboard...');
            const result = await window.electronAPI.uploadSteamScore(
                'EpicenterCity_Score',  // Leaderboard name
                Math.round(finalScore)   // Convert to integer for Steam
            );

            if (result.success) {
                console.log('✓ Score successfully uploaded to Steam!');
            } else {
                console.log(`⚠ Steam upload failed: ${result.message}`);
                if (result.fallback) {
                    console.log('  (Score saved to local storage as fallback)');
                }
            }
        } catch (error) {
            ...
            leaderboardData.forEach((entry, index) => {
                ...
                div.innerHTML = `
                    <div style="display: flex; justify-content: space-between; margin-bottom: 5px;">
                        ...
                        <span style="color: #cccccc;">${entry.username || 'Unknown'}</span>
